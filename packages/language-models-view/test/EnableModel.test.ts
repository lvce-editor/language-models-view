...
test('enableModel enables a model by id', () => {
  const state = {
    ...createDefaultState(),
    models: [
      { enabled: false, id: 'gpt-4', inputContextSize: 8192, name: 'GPT-4', outputContextSize: 4096, provider: 'openai', selected: false },
...
test('enableModel returns unchanged state if model id not found', () => {
  const state = {
    ...createDefaultState(),
    models: [{ enabled: false, id: 'gpt-4', inputContextSize: 8192, name: 'GPT-4', outputContextSize: 4096, provider: 'openai', selected: false }],
  }
